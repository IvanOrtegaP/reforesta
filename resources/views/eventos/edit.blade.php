<div>
    <form action="{{ route('eventos.update', $evento->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion">{{ old('descripcion', $evento->descripcion) }}</textarea>
            @error('descripcion')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="t_terreno" name="t_terreno" value="{{ old('t_terreno', $evento->t_terreno) }}">
            @error('t_terreno')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
            ...
            <select id="participantes" name="participantes[]" multiple>
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id }}"
                        {{ (collect(old('participantes', $evento->participantes->pluck('id')->toArray()))->contains($usuario->id)) ? 'selected' : '' }}>
                        {{ $usuario->nick }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            @foreach($especies as $especie)
                @php
                    $checked = old("especies.{$especie->id}.id")
                        ? true
                        : (isset($evento->especies) && $evento->especies->contains('id', $especie->id));
                    $pivot = isset($evento->especies)
                        ? $evento->especies->firstWhere('id', $especie->id)?->pivot
                        : null;
                    $cantidad = old("especies.{$especie->id}.cantidad") ?? ($pivot ? $pivot->cantidad : '');
                @endphp
                <div>
